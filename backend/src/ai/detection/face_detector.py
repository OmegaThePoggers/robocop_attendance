...
class FaceDetector:
    ...
    def detect(self, image: np.ndarray, min_face_size: int = MIN_FACE_SIZE_PX) -> Tuple[list, np.ndarray]:
        """
        Returns (faces, resized_image).
        Faces smaller than min_face_size are discarded.
        Each face object carries .bbox, .kps (5 landmarks), .normed_embedding, .det_score.
        """
        image = self._resize(image)
        faces = self._app.get(image)
        faces = [f for f in faces if (f.bbox[3] - f.bbox[1]) >= min_face_size]
        return faces, image
    ...
    def detect_and_align(self, image: np.ndarray, min_face_size: int = MIN_FACE_SIZE_PX) -> Tuple[List[np.ndarray], list, np.ndarray]:
        """
        Full detection + alignment pass.
        Returns (aligned_crops, raw_faces, resized_image).
        """
        faces, resized = self.detect(image, min_face_size=min_face_size)
