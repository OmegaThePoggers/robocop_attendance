...
import { registerTeacher, registerUser } from '../../lib/api';
        ...
        <AppShell>
            <div className="min-h-[80vh] flex items-center justify-center py-10 px-4 sm:px-6 lg:px-8 relative z-10 w-full">
                <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-5 glass-panel-heavy overflow-hidden">
                    ...
                    <div className="hidden md:flex flex-col justify-between p-10 bg-linear-to-br from-primary-900/40 to-dark-bg border-r border-dark-border/50 md:col-span-2 relative overflow-hidden">
                        <div className="absolute -top-24 -left-24 w-48 h-48 bg-primary-600/20 rounded-full blur-3xl"></div>
                        <div className="relative z-10">
                            <div className="w-12 h-12 rounded-xl bg-linear-to-br from-primary-500 to-primary-600 flex items-center justify-center shadow-lg shadow-primary-500/20 mb-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" className="input-field" value={fullName} placeholder="John Doe" onChange={e => setFullName(e.target.value)} />
                                </div>
                                <div>
                                    <label className="label-text">Username</label>
                                    <input type="text" className="input-field" value={username} placeholder="johndoe" onChange={e => setUsername(e.target.value)} />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="password" className="input-field" value={password} placeholder="••••••••" onChange={e => setPassword(e.target.value)} />
                                </div>
                                <div>
                                    <label className="label-text">{role === 'student' ? 'SAP ID' : 'Teacher ID'}</label>
                                    <input type="text" className="input-field" value={idNumber} placeholder={role === 'student' ? "e.g. 70322300066" : "e.g. TCH-1234"} onChange={e => setIdNumber(e.target.value)} />
                                </div>
                            </div>
                                    ...
                                    {!photoPreview && !cameraActive && (
                                        <div className="w-full border-2 border-dashed border-dark-border rounded-xl p-8 flex flex-col items-center justify-center gap-4 bg-dark-bg/30 hover:bg-dark-bg/50 transition-colors group">
                                            <div className="flex gap-4">
                                                <button type="button" onClick={startCamera} className="flex flex-col items-center gap-2 p-4 rounded-lg bg-dark-border/30 hover:bg-primary-600/20 hover:text-primary-400 transition-colors text-dark-muted border border-transparent hover:border-primary-500/30">
                                                ...
                                                <button type="button" onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-2 p-4 rounded-lg bg-dark-border/30 hover:bg-primary-600/20 hover:text-primary-400 transition-colors text-dark-muted border border-transparent hover:border-primary-500/30">
                                    ...
                                    {cameraActive && (
                                        <div className="space-y-4 animate-fade-in">
                                            ...
                                            <div className="flex gap-3">
                                                <button type="button" onClick={capturePhoto} className="flex-1 btn-primary py-2.5">Capture Photo</button>
                                                <button type="button" onClick={stopCamera} className="px-6 btn-secondary py-2.5">Cancel</button>
                                            </div>
                                    ...
                                    {photoPreview && (
                                        <div className="space-y-4 animate-fade-in">
                                            <div className="aspect-video bg-black rounded-xl overflow-hidden border border-emerald-500/50 relative">
                                                ...
                                                <div className="absolute top-3 right-3 bg-emerald-500/90 backdrop-blur-sm text-black text-xs font-bold px-3 py-1.5 rounded-md flex items-center gap-1.5 shadow-lg">
                            ...
                            <p className="text-center text-sm text-dark-muted mt-6">
                                ...
                                <button type="button" onClick={() => router.push('/login')} className="text-primary-400 hover:text-primary-300 font-medium transition-colors">
                                    Sign in
                                </button>
                            </p>
                        </form>
